...
import type { ICartModuleService, IRegionModuleService, IProductModuleService } from "@medusajs/framework/types";
...
export function getCartTools(container: MedusaContainer) {
  return [
    tool(
      async ({ customer_id }: { customer_id: string }) => {
        try {
          ...
          // Get default region
          const [regions] = await regionModule.listAndCountRegions({}, { take: 1 });
    ...
    tool(
        async ({ cart_id, variant_id, quantity }: { cart_id: string; variant_id: string; quantity: number }) => {
            try {
                const cartModule: ICartModuleService = container.resolve(Modules.CART);
