    ...
    >
      <CardHeader className="flex-col items-start gap-2 pt-10 px-8 pb-0">
        <div className="flex justify-between w-full items-center mb-2">
            <h3 className={`text-2xl font-black tracking-tight ${popular ? "text-primary" : "text-default-700"}`}>
                {name}
            </h3>
            {popular && (
            <Chip color="primary" variant="shadow" size="sm" className="font-bold uppercase tracking-wider">
                Most Popular
            </Chip>
            )}
        </div>
        ...
        <p className="text-sm text-default-500 mt-4 leading-relaxed font-medium">
            Perfect for {name === "Free" ? "early testing and enthusiasts" : name === "Starter" ? "growing modern businesses" : "demanding large scale operations"}.
        </p>
      </CardHeader>
        ...
        <ul className="flex flex-col gap-4">
          {features.map((feature, i) => (
            <li key={i} className="flex items-start gap-4">
              <div className={`mt-1 flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${popular ? "bg-primary/10 text-primary" : "bg-default-100 text-default-600"}`}>
                 <Check size={12} strokeWidth={3} />
              </div>
              <span className="text-sm text-default-600 font-medium leading-tight">{feature}</span>
            </li>
          ))}
        </ul>
      </CardBody>
