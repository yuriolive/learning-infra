...
describe("CloudRunProvider", () => {
  ...
  describe("startDeployTenantInstance", () => {
    ...
    it("should set IAM policy for public access after creating service", async () => {
      mockRunClient.projects.locations.services.get.mockRejectedValueOnce({
        code: 404,
      });

      await provider.startDeployTenantInstance("tenant-1", {
        databaseUrl: "postgres://...",
        redisUrl: "redis://...",
        redisPrefix: "t_1:",
        subdomain: "tenant-1",
      ...
      expect(
        mockRunClient.projects.locations.services.getIamPolicy,
      ).toHaveBeenCalledWith({
        resource:
          "projects/test-project/locations/us-central1/services/tenant-tenant-1",
      });
      expect(
    ...
    it("should preserve existing IAM bindings when adding allUsers", async () => {
      mockRunClient.projects.locations.services.get.mockRejectedValueOnce({
        code: 404,
      });
      ...
      const setIamCall =
        mockRunClient.projects.locations.services.setIamPolicy.mock
          .calls[0]?.[0];
      expect(setIamCall).toBeDefined();
      expect(setIamCall?.requestBody.policy.bindings).toHaveLength(2);
      expect(setIamCall?.requestBody.policy.bindings).toEqual(
